...
class Arrays
{
	 ...
	 * @param  ?T  $default
	 * @return ?T
	 * @throws MvpDocs\Vendor\Nette\InvalidArgumentException if item does not exist and default value is not provided
	 */
	public static function get(array $array, string|int|array $key, mixed $default = null): mixed
	{
		foreach (is_array($key) ? $key : [$key] as $k) {
			if (is_array($array) && array_key_exists($k, $array)) {
				...
				if (func_num_args() < 3) {
					throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Missing item '$k'.");
				}
	 ...
	 * @param  array-key|array-key[]  $key
	 * @return ?T
	 * @throws MvpDocs\Vendor\Nette\InvalidArgumentException if traversed item is not an array
	 */
	public static function &getRef(array &$array, string|int|array $key): mixed
	{
		foreach (is_array($key) ? $key : [$key] as $k) {
			if (is_array($array) || $array === null) {
				...
				throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('Traversed item is not an array.');
			}
		}
	...
	public static function associate(array $array, string|array $path): array|\stdClass
	{
		...
		if (!$parts || $parts === ['->'] || $parts[0] === '=' || $parts[0] === '|') {
			throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Invalid path '" . (is_array($path) ? implode('', $path) : $path) . "'.");
		}
	...
	public static function pick(array &$array, string|int $key, mixed $default = null): mixed
	{
		if (array_key_exists($key, $array)) {
			...
			throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Missing item '$key'.");
