...
class Image
{
	use MvpDocs\Vendor\Nette\SmartObject;
	 ...
	 * @throws MvpDocs\Vendor\Nette\NotSupportedException if gd extension is not loaded
	...
	public static function fromBlank(int $width, int $height, ImageColor|array|null $color = null): static
	{
		self::ensureExtension();
		if ($width < 1 || $height < 1) {
			throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('Image width and height must be greater than zero.');
		}
	...
	public static function typeToExtension(int $type): string
	{
		if (!isset(self::Formats[$type])) {
			throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Unsupported image type '$type'.");
		}
	...
	public static function extensionToType(string $extension): int
	{
		...
		if (!isset($extensions[$extension])) {
			throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Unsupported file extension '$extension'.");
		}
	...
	public function resize(int|string|null $width, int|string|null $height, int $mode = self::OrSmaller): static
	{
		if ($mode & self::Cover) {
			if ($width === null || $height === null) {
				throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('Both width and height must be set for Cover mode.');
			}
	...
	): array
	{
		...
		if ($mode & self::Stretch) { // non-proportional
			if (!$newWidth || !$newHeight) {
				throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('For stretching must be both width and height specified.');
			}
			...
			if (!$newWidth && !$newHeight) {
				throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('At least width or height must be specified.');
			}
	...
	private function output(int $type, ?int $quality, ?string $file = null): void
	{
		[$defQuality, $min, $max] = match ($type) {
			...
			default => throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Unsupported image type '$type'."),
		};
	 ...
	 * @param  mixed[]  $args
	 * @throws MvpDocs\Vendor\Nette\MemberAccessException
	 */
	...
	public function __clone()
	{
		...
		$this->setImageResource(imagecreatefromstring(ob_get_clean()) ?: throw new MvpDocs\Vendor\Nette\ShouldNotHappenException);
	}
	...
	private static function isPercent(int|string &$num): bool
	{
		...
		throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Expected dimension in int|string, '$num' given.");
	}
	...
	{
		throw new \MvpDocs\Vendor\Nette\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
	}
	...
	private static function ensureExtension(): void
	{
		if (!extension_loaded('gd')) {
			throw new \MvpDocs\Vendor\Nette\NotSupportedException('PHP extension GD is not loaded.');
		}
	}
}
