...
final class Structure implements Schema
{
	...
	{
		throw new \MvpDocs\Vendor\Nette\InvalidStateException('Structure cannot have default value.');
	}
		...
		if ($extraKeys = array_keys(array_diff_key($value, $items))) {
			if ($this->otherItems) {
				...
				foreach ($extraKeys as $key) {
					$hint = MvpDocs\Vendor\Nette\Utils\Helpers::getSuggestion($keys, (string) $key);
					$context->addError(
						'Unexpected item %path%' . ($hint ? ", did you mean '%hint%'?" : '.'),
						MvpDocs\Vendor\Nette\Schema\Message::UnexpectedItem,
						['hint' => $hint],
					)->path[] = $key;
				}
			}
