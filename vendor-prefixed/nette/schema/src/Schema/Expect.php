...
use MvpDocs\Vendor\Nette\Schema\Elements\AnyOf;
use MvpDocs\Vendor\Nette\Schema\Elements\Structure;
use MvpDocs\Vendor\Nette\Schema\Elements\Type;
...
final class Expect
{
		...
		foreach ($props as $prop) {
			...
			if (!isset($items[$name])) {
				...
				if ($prop instanceof \ReflectionProperty ? $prop->isInitialized($object) : $prop->isOptional()) {
					...
					} elseif ($def === null && !MvpDocs\Vendor\Nette\Utils\Validators::is(null, $type)) {
						$item->required();
					} else {
						$item->default($def);
					}
	...
	{
		$shape ??= [];
		return MvpDocs\Vendor\Nette\Utils\Arrays::first($shape) instanceof Schema
