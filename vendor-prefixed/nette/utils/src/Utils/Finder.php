...
class Finder implements \IteratorAggregate
{
	...
	private function addMask(array $masks, string $mode): static
	{
		foreach ($masks as $mask) {
			...
			if ($mask === '' || ($mode === 'file' && str_ends_with($mask, '/'))) {
				throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Invalid mask '$mask'");
			}
	...
	private function addLocation(array $paths, string $ext): void
	{
		foreach ($paths as $path) {
			if ($path === '') {
				throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Invalid directory '$path'");
			}
	...
	public function size(string $operator, ?int $size = null): static
	{
		if (func_num_args() === 1) { // in $operator is predicate
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?((?:\d*\.)?\d+)\s*(K|M|G|)B?$#Di', $operator, $matches)) {
				throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('Invalid size predicate format.');
			}
	...
	public function date(string $operator, string|int|\DateTimeInterface|null $date = null): static
	{
		if (func_num_args() === 1) { // in $operator is predicate
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?(.+)$#Di', $operator, $matches)) {
				throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('Invalid date predicate format.');
			}
	...
	private function traverseDir(string $dir, array $searches, array $subdirs = []): \Generator
	{
		if ($this->maxDepth >= 0 && count($subdirs) > $this->maxDepth) {
			...
			throw new \MvpDocs\Vendor\Nette\InvalidStateException(sprintf("Directory '%s' does not exist.", rtrim($dir, '/\\')));
		}
		...
		try {
			...
			if ($this->ignoreUnreadableDirs) {
				...
				throw new \MvpDocs\Vendor\Nette\InvalidStateException($e->getMessage());
			}
		}
	...
	private function buildPlan(): array
	{
		...
		foreach ($this->find as [$mask, $mode]) {
			...
			if (FileSystem::isAbsolute($mask)) {
				if ($this->in) {
					throw new \MvpDocs\Vendor\Nette\InvalidStateException("You cannot combine the absolute path in the mask '$mask' and the directory to search '{$this->in[0]}'.");
				}
			...
			foreach ($splits as [$base, $rest, $recursive]) {
				...
				if (!$dirs) {
					throw new \MvpDocs\Vendor\Nette\InvalidStateException(sprintf("Directory '%s' does not exist.", rtrim($base, '/\\')));
				}
