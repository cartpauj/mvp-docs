...
final class Reflection
{
	...
	/** @deprecated use MvpDocs\Vendor\Nette\Utils\Validators::isBuiltinType() */
	...
	#[\Deprecated('use MvpDocs\Vendor\Nette\Utils\Validators::isClassKeyword()')]
	...
	public static function getParameterDefaultValue(\ReflectionParameter $param): mixed
	{
		if ($param->isDefaultValueConstant()) {
			$const = $orig = $param->getDefaultValueConstantName() ?? throw new MvpDocs\Vendor\Nette\ShouldNotHappenException;
			$pair = explode('::', $const);
			if (isset($pair[1])) {
	...
	public static function toString(\Reflector $ref): string
	{
		if ($ref instanceof \ReflectionClass) {
			...
			throw new MvpDocs\Vendor\Nette\InvalidArgumentException;
		}
	}
	 ...
	 * @throws MvpDocs\Vendor\Nette\InvalidArgumentException
	 */
	public static function expandClassName(string $name, \ReflectionClass $context): string
	{
		$lower = strtolower($name);
		if (empty($name)) {
			throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('Class name must not be empty.');
	...
	public static function getUseStatements(\ReflectionClass $class): array
	{
		if ($class->isAnonymous()) {
			throw new \MvpDocs\Vendor\Nette\NotImplementedException('Anonymous classes are not supported.');
		}
