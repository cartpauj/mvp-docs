...
namespace MvpDocs\Vendor\Nette\Schema\Elements;

use MvpDocs\Vendor\Nette;
use MvpDocs\Vendor\Nette\Schema\Context;
use MvpDocs\Vendor\Nette\Schema\Helpers;
...
final class AnyOf implements Schema
{
	...
	{
		if (!$set) {
			throw new \MvpDocs\Vendor\Nette\InvalidStateException('The enumeration must not be empty.');
		}
	...
	{
		$this->set[] = new Type(MvpDocs\Vendor\Nette\Schema\DynamicParameter::class);
		return $this;
	}
	...
	private function findAlternative(mixed $value, Context $context): mixed
	{
		...
		foreach ($this->set as $item) {
			if ($item instanceof Schema) {
				...
				$expecteds[] = MvpDocs\Vendor\Nette\Schema\Helpers::formatValue($item);
			}
		}
		...
		if ($innerErrors) {
			...
			$context->addError(
				...
				MvpDocs\Vendor\Nette\Schema\Message::TypeMismatch,
				[
					'value' => $value,
	...
	public function completeDefault(Context $context): mixed
	{
		if ($this->required) {
			$context->addError(
				...
				MvpDocs\Vendor\Nette\Schema\Message::MissingItem,
			);
			return null;
		}
