...
use MvpDocs\Vendor\Nette\Utils\Reflection;
...
final class Helpers
{
	...
	{
		if ($type = MvpDocs\Vendor\Nette\Utils\Type::fromReflection($prop)) {
			return (string) $type;
		} elseif (
	...
	public static function parseAnnotation(\ReflectionClass|\ReflectionProperty $ref, string $name): ?string
	{
		if (!Reflection::areCommentsAvailable()) {
			throw new \MvpDocs\Vendor\Nette\InvalidStateException('You have to enable phpDoc comments in opcode cache.');
		}
	...
	public static function formatValue(mixed $value): string
	{
		if ($value instanceof DynamicParameter) {
			...
			return "'" . MvpDocs\Vendor\Nette\Utils\Strings::truncate($value, 15, '...') . "'";
	...
	{
		if (!MvpDocs\Vendor\Nette\Utils\Validators::is($value, $expected)) {
		...
		if (is_array($value) || is_string($value)) {
			[$length, $label] = is_array($value)
				...
				: (in_array('unicode', explode('|', $types), strict: true)
					? [MvpDocs\Vendor\Nette\Utils\Strings::length($value), 'characters']
					: [strlen($value), 'bytes']);
	...
	{
		if (MvpDocs\Vendor\Nette\Utils\Validators::isBuiltinType($type)) {
			...
			return static fn($value) => MvpDocs\Vendor\Nette\Utils\Arrays::toObject((array) $value, new $type);
		}
	}
}
