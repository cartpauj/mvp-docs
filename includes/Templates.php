...
if ( wp_is_block_theme() ) {
	add_action( 'init', function () {
		...
		$single_file = MVPD_PATH . 'templates/block/single-mvpd_doc.html';
		if ( file_exists( $single_file ) ) {
			register_block_template( 'mvp-docs//single-mvpd_doc', [
	...
	// Tell WordPress to use our search-mvpd_doc block template for the search page.
	add_filter( 'pre_get_block_file_template', function ( $template, $id, $template_type ) {
		...
		$block_template = get_block_template( 'mvp-docs//search-mvpd_doc' );
	...
	add_filter( 'archive_template_hierarchy', function ( $templates ) {
		if ( get_query_var( 'mvpd_search' ) ) {
			array_unshift( $templates, 'search-mvpd_doc' );
		}
		return $templates;
	} );
} else {
	add_filter( 'template_include', function ( $template ) {
		if ( is_singular( 'mvpd_doc' ) ) {
			$override = MVPD_PATH . 'templates/classic/single-mvpd_doc.php';
			if ( file_exists( $override ) ) {
				return $override;
			}
		}

		if ( get_query_var( 'mvpd_search' ) || is_post_type_archive( 'mvpd_doc' ) || is_tax( 'mvpd_category' ) ) {
			$override = MVPD_PATH . 'templates/classic/archive-mvpd_doc.php';
			if ( file_exists( $override ) ) {
				return $override;
			}
		}
