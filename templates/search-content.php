<?php
/**
 * Search results content — rendered via [mvpd_search] shortcode.
 * Shows breadcrumbs and a full-width card listing matching docs.
 *
 * @package MVP_Docs
 * @var string   $search_query The sanitized search query.
 * @var WP_Query $search_docs  Query of matching docs.
 */
...
<div class="mvpd-archive">
	...
	<div class="mvpd-search" role="search">
		...
		<input type="text" class="mvpd-search-input" placeholder="<?php esc_attr_e( 'Search docs…', 'mvp-docs' ); ?>" value="<?php echo esc_attr( $search_query ); ?>" autocomplete="off" />
		...
		<div class="mvpd-card">
			<div class="mvpd-card-header">
				<div class="mvpd-card-header-top">
					<h2 class="mvpd-card-title"><?php
						/* translators: %s: search query */
						printf( esc_html__( 'Results for "%s"', 'mvp-docs' ), esc_html( $search_query ) );
					?></h2>
					<span class="mvpd-card-badge"><?php echo esc_html( $search_docs->found_posts ); ?></span>
				</div>
			</div>
			<?php if ( $search_docs->have_posts() ) : ?>
				<ul class="mvpd-card-list">
					<?php while ( $search_docs->have_posts() ) : $search_docs->the_post(); ?>
						<li>
			...
			<?php else : ?>
				<p class="mvpd-card-empty"><?php esc_html_e( 'No docs found.', 'mvp-docs' ); ?></p>
			<?php endif; ?>
		</div>
	</div>
</div>
