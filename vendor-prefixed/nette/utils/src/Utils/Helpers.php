...
class Helpers
{
	...
	{
		if ($min > $max) {
			throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Minimum ($min) is not less than maximum ($max).");
		}
	...
	public static function compare(mixed $left, string $operator, mixed $right): bool
	{
		return match ($operator) {
			...
			'!=', '<>' => $left != $right,
			'!==' => $left !== $right,
			default => throw new \MvpDocs\Vendor\Nette\InvalidArgumentException("Unknown operator '$operator'"),
		};
	}
