...
final class FileSystem
{
	 ...
	 * @throws MvpDocs\Vendor\Nette\IOException  on error occurred
	 */
	public static function createDir(string $dir, int $mode = 0o777): void
	{
		if (!is_dir($dir) && !@mkdir($dir, $mode, recursive: true) && !is_dir($dir)) { // @ - dir may already exist
			throw new \MvpDocs\Vendor\Nette\IOException(sprintf(
	 ...
	 * @throws MvpDocs\Vendor\Nette\IOException  on error occurred
	 * @throws MvpDocs\Vendor\Nette\InvalidStateException  if $overwrite is set to false and destination already exists
	 */
	public static function copy(string $origin, string $target, bool $overwrite = true): void
	{
		if (stream_is_local($origin) && !file_exists($origin)) {
			throw new \MvpDocs\Vendor\Nette\IOException(sprintf("File or directory '%s' not found.", self::normalizePath($origin)));

		} elseif (!$overwrite && file_exists($target)) {
			throw new \MvpDocs\Vendor\Nette\InvalidStateException(sprintf("File or directory '%s' already exists.", self::normalizePath($target)));
	...
	public static function readLines(string $file, bool $stripNewLines = true): \Generator
	{
		return (function ($f) use ($file, $stripNewLines) {
			$counter = 0;
			do {
				$line = Callback::invokeSafe('fgets', [$f], fn($error) => throw new \MvpDocs\Vendor\Nette\IOException(sprintf(
					"Unable to read file '%s'. %s",
					self::normalizePath($file),
					$error,
				)));
	...
	public static function makeWritable(string $path, int $dirMode = 0o777, int $fileMode = 0o666): void
	{
		if (is_file($path)) {
			...
			throw new \MvpDocs\Vendor\Nette\IOException(sprintf("File or directory '%s' not found.", self::normalizePath($path)));
		}
	}
