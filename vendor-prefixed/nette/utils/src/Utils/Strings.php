...
class Strings
{
	use MvpDocs\Vendor\Nette\StaticClass;
	...
	/**
	 * @deprecated use MvpDocs\Vendor\Nette\Utils\Validators::isUnicode()
	 */
	 ...
	 * @throws MvpDocs\Vendor\Nette\InvalidArgumentException if code point is not in valid range
	 */
	public static function chr(int $code): string
	{
		if ($code < 0 || ($code >= 0xD800 && $code <= 0xDFFF) || $code > 0x10FFFF) {
			throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('Code point must be in range 0x0 to 0xD7FF or 0xE000 to 0x10FFFF.');
		} elseif (!extension_loaded('iconv')) {
			throw new \MvpDocs\Vendor\Nette\NotSupportedException(__METHOD__ . '() requires ICONV extension that is not loaded.');
		}

		$res = iconv('UTF-32BE', 'UTF-8//IGNORE', pack('N', $code));
		return $res === false ? throw new MvpDocs\Vendor\Nette\ShouldNotHappenException : $res;
	}
	...
	public static function ord(string $c): int
	{
		...
		if ($tmp === false || $tmp === '') {
			throw new \MvpDocs\Vendor\Nette\InvalidArgumentException('Invalid UTF-8 character "' . ($c === '' ? '' : '\x' . strtoupper(bin2hex($c))) . '".');
		}

		return unpack('N', $tmp)[1] ?? throw new MvpDocs\Vendor\Nette\ShouldNotHappenException;
	}
	...
	public static function substring(string $s, int $start, ?int $length = null): string
	{
		if (function_exists('mb_substr')) {
			...
			throw new \MvpDocs\Vendor\Nette\NotSupportedException(__METHOD__ . '() requires extension ICONV or MBSTRING, neither is loaded.');
		} elseif ($length === null) {
		...
		return $res === false ? throw new \MvpDocs\Vendor\Nette\InvalidStateException('iconv_substr() failed.') : $res;
	}
	...
	public static function toAscii(string $s): string
	{
		if (!extension_loaded('intl')) {
			throw new \MvpDocs\Vendor\Nette\NotSupportedException(__METHOD__ . '() requires INTL extension that is not loaded.');
		}
		...
		$s = \Transliterator::create('Any-Latin; Latin-ASCII')?->transliterate($s)
			?? throw new \MvpDocs\Vendor\Nette\InvalidStateException('Transliterator::transliterate() failed.');
		...
		if ($iconv === 'glibc') {
			...
			if ($s === false) {
				throw new \MvpDocs\Vendor\Nette\InvalidStateException('iconv() failed.');
			}
	...
	): string
	{
		if (is_object($replacement) || is_array($replacement)) {
			if (!is_callable($replacement, false, $textual)) {
				throw new \MvpDocs\Vendor\Nette\InvalidStateException("Callback '$textual' is not callable.");
			}
